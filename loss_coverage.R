...
var_razdep_idosos = razdep_bh_2010 / razdep_bh_2000[2,2]
...
LC = function(vetor_premio,vetor_risco_bh,tau,lambda){
  ...
  pii = vetor_risco_br * vetor_premio
  ...
  expected_loss_coverage = sum(demand_function(lambda)*p*pii*vetor_risco_br)
...
prop$REGRA_5 = NA
  ...
  prop$REGRA_1[1:10] = seq(min(prop$PREMIO_MEDIO[1:10]), var_razdep_idosos*max(prop$PREMIO_MEDIO[1:10]), length.out = 10)

  #---------REGRA 2: variacao acumulada entre f1 e f7 < variacao acumulada entre f7 e f10
  prop$REGRA_2[1:10] = c(prop$PREMIO_MEDIO[1:7]*1.9, prop$PREMIO_MEDIO[8:10])

  #---------REGRA 3: variacao acumulada entre f1 e f7 < variacao acumulada entre f7 e f10
  prop$REGRA_3[1:10] = c(prop$PREMIO_MEDIO[1],rep(prop$PREMIO_MEDIO[2],8),prop$PREMIO_MEDIO[10])
  
  #---------REGRA 4:
  prop$REGRA_4[1:10] = rep(mean(prop$PREMIO_MEDIO[1:10]),10)
  
  #---------REGRA 5:
  prop$REGRA_5[1:10] = 1000*prop$RISCO_RUSSAM_MEDIO[1:10]
...
alfa = data.frame(LC_REGRA_ANS,LC_REGRA_1, LC_REGRA_2,LC_REGRA_3,LC_REGRA_4, LC_REGRA_5)
...
russam = data.frame(LC_REGRA_ANS,LC_REGRA_1, LC_REGRA_2,LC_REGRA_3,LC_REGRA_4, LC_REGRA_5)
...
d = rbind(d, r)
d$Risco = rep(c('alfa','russam'), each = 900)
...
G_ALFA = ggplot(d[!(d$Regra == 'REGRA_ANS') & d$Risco == 'russam',], aes(Lambda, LC, color = Regra)) + geom_line() +
  theme_bw()  +
  theme(axis.line = element_line(size=1, colour = "black"),
        panel.grid.major = element_line(), panel.grid.minor = element_blank(),
        panel.border = element_blank(), panel.background = element_blank(), 
  ...
  ggtitle(expression('Gráfico utilizando o risco'~alpha))
...
G_RUSSAM = ggplot(d[!(d$Regra == 'REGRA_ANS') & d$Risco == 'russam',], aes(Lambda, LC, color = Regra)) + geom_line()  +
  ...
  xlab(expression(lambda)) + ylab('LC') + scale_color_manual(labels = c(paste0('Regra ',1:5)), values = c('red','blue','black','darkgreen','purple')) +
  annotate("text", x=0.8, y=1.05, label= "Regra ANS = 1") + 
  ggtitle('Gráfico utilizando o risco Russam')
